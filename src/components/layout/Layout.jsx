...
import {
  GraduationCap, LayoutDashboard, Users, CheckSquare,
...
export default function Layout({ page, setPage, children }) {
  ...
  const NavLink = ({ item }) => {
    ...
    return (
      <button onClick={() => { setPage(item.id); setOpen(false) }}
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group
          ${active ? 'bg-brand-600 text-white shadow-md shadow-brand-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}>
  ...
  const Sidebar = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 pb-4 border-t border-slate-100 pt-3 space-y-1 shrink-0">
        {NAV_BOTTOM.map(item => <NavLink key={item.id} item={item} />)}
        <div className="flex items-center gap-3 px-2 py-2 mt-1">
          <div className="w-8 h-8 rounded-full bg-brand-100 flex items-center justify-center shrink-0">
            <span className="text-brand-700 font-bold text-sm">{(profile?.full_name || 'G').charAt(0).toUpperCase()}</span>
          </div>
