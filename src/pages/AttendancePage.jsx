...
import { CheckCircle, Clock, AlertCircle, XCircle, ChevronLeft, ChevronRight, Save, Loader, Users, BarChart2, Calendar, FileText } from 'lucide-react'

const STATUS_CFG = {
  Hadir: { color: 'bg-emerald-500', light: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: CheckCircle },
  Izin:  { color: 'bg-sky-500',     light: 'bg-sky-50 text-sky-700 border-sky-200',             icon: Clock      },
  Sakit: { color: 'bg-amber-500',   light: 'bg-amber-50 text-amber-700 border-amber-200',       icon: AlertCircle },
...
export default function AttendancePage() {
  ...
  const loadAtt = useCallback(async () => {
    ...
    const { data } = await supabase.from('attendance').select('student_id,status,notes').eq('date', date).in('student_id', ids)
    const map = {}
    ;(data||[]).forEach(r => { map[r.student_id] = { status: r.status, notes: r.notes||'' } })
    setAtt(map); setLoading(false)
  }, [date, students])
  ...
  const loadRekap = useCallback(async () => {
    ...
    const { data } = await supabase.from('attendance').select('student_id,date,status')
      .in('student_id', ids).gte('date', start).lt('date', toISO(endD)).order('date')
    const rows   = data || []
  ...
  const markAll    = (status) => { const up = {}; filtered.forEach(s => { up[s.id] = { status, notes: att[s.id]?.notes||'' } }); setAtt(p => ({...p,...up})) }
  const handleSave = async () => {
    setSaving(true)
    const records = filtered.filter(s=>att[s.id]?.status).map(s => ({ student_id:s.id, date, status:att[s.id].status, notes:att[s.id].notes||null }))
  ...
  return (
    <div className="space-y-5 animate-fadeUp">
      <div className="flex items-center justify-between">
        <div><h1 className="font-display text-2xl font-bold text-slate-800">Absensi</h1><p className="text-slate-500 text-sm">Input dan rekap kehadiran siswa</p></div>
        <div className="flex bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
          {[['input', Calendar, 'Input'], ['rekap', BarChart2, 'Rekap']].map(([id, Icon, label]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold transition-colors ${tab===id ? 'bg-brand-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
              <Icon size={15} /> {label}
            </button>
          ))}
        </div>
        ...
        <>
          <div className="card p-3 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-0.5 bg-slate-50 rounded-xl px-1 py-1 border border-slate-100">
                <button onClick={()=>shiftDate(-1)} className="p-1.5 hover:bg-white rounded-lg transition-colors"><ChevronLeft size={15} className="text-slate-500" /></button>
                <input type="date" value={date} onChange={e=>setDate(e.target.value)} className="bg-transparent text-sm font-semibold text-slate-700 focus:outline-none w-[130px]" />
                <button onClick={()=>shiftDate(1)} className="p-1.5 hover:bg-white rounded-lg transition-colors"><ChevronRight size={15} className="text-slate-500" /></button>
              </div>
              <button onClick={()=>setDate(toISO(new Date()))} className="text-xs font-semibold text-brand-600 bg-brand-50 hover:bg-brand-100 px-2.5 py-1.5 rounded-lg transition-colors">Hari Ini</button>
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {STATUSES.map(s => (
                  <button key={s} onClick={()=>markAll(s)} className={`text-xs px-2.5 py-1.5 rounded-lg border font-bold transition-colors ${STATUS_CFG[s].light}`}>{s}</button>
                ))}
              </div>
            </div>
            ...
            {STATUSES.map(s => {
              ...
              return (
                <div key={s} className="card px-4 py-3 flex items-center gap-3">
                  ...
                  <div><div className="font-display text-xl font-bold text-slate-800">{stats[s]}</div><div className="text-xs text-slate-500">{s} · {pct}%</div></div>
                </div>
              )
            })}
          </div>
          ...
          {filtered.length > 0 && (
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                <div className="bg-brand-500 h-full rounded-full transition-all" style={{ width: `${(filled/filtered.length)*100}%` }} />
              </div>
              <span className="text-xs text-slate-500 font-medium shrink-0">{filled}/{filtered.length}{filled<filtered.length && <span className="text-amber-500 ml-1">({filtered.length-filled} belum)</span>}</span>
            </div>
          )}
          ...
          <div className="card overflow-hidden">
            ...
            <div className="hidden md:flex items-center bg-slate-50 border-b border-slate-100 px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">
              ...
              <div className="w-[272px] shrink-0">Status</div>
                  ...
                  return (
                    <div key={s.id}
                      className={`px-5 transition-colors hover:bg-slate-50/70 ${!status?'bg-amber-50/20':''}
                        py-3 flex flex-col gap-2.5 md:flex-row md:items-center md:py-2.5 md:gap-0`}>
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0 md:pr-4">
                        ...
                        {status && <span className={`md:hidden shrink-0 text-xs font-bold px-2 py-0.5 rounded-lg border ${STATUS_CFG[status].light}`}>{status}</span>}
                      </div>
                      <div className="flex gap-1.5 md:w-[272px] md:shrink-0">
                        {STATUSES.map(st => (
                          <button key={st} onClick={()=>setStatus(s.id,st)}
                            className={`flex-1 py-1.5 md:py-2 rounded-lg text-xs font-bold border transition-all
                              ${status===st ? `${STATUS_CFG[st].color} text-white border-transparent shadow-sm` : 'border-slate-200 text-slate-400 hover:border-slate-300 hover:bg-slate-50'}`}>
                            {st}
                          </button>
                        ))}
                      </div>
                      <div className="md:flex-1 md:min-w-0 md:pl-4">
                        <input value={notes} onChange={e=>setNotes(s.id,e.target.value)} placeholder="Catatan..." className="input text-xs py-1.5 w-full" />
                      </div>
                    </div>
                  )
                })}
          ...
          {filtered.length > 0 && (
            <div className="flex justify-end pb-4">
              <button onClick={handleSave} disabled={saving||filled===0} className={`btn-primary flex items-center gap-2 px-8 ${saved?'!bg-emerald-500':''}`}>
      ...
      {tab === 'rekap' && (
        <>
          <div className="card p-4 flex flex-wrap items-end gap-4">
            <div><label className="label">Bulan</label><input type="month" className="input w-auto" value={rekapMonth} onChange={e=>setRekapMonth(e.target.value)} /></div>
            ...
            <button onClick={loadRekap} className="btn-primary flex items-center gap-2"><BarChart2 size={15} /> Tampilkan</button>
          </div>

          {rekapLoading ? (
            <div className="flex justify-center py-20"><Loader size={28} className="text-brand-500 animate-spin" /></div>
          ) : rekapData.length === 0 ? (
            <div className="card text-center py-16 text-slate-400"><FileText size={40} className="mx-auto mb-3 opacity-30" /><p>Pilih bulan &amp; kelas, lalu klik Tampilkan</p></div>
          ) : (
            <>
                ...
                {STATUSES.map(s => {
                  ...
                  return (
                    <div key={s} className="card p-4 flex items-center gap-3">
                      ...
                      <div><div className="font-display text-2xl font-bold text-slate-800">{total}</div><div className="text-xs text-slate-500">Total {s}</div></div>
                    </div>
                  )
                })}
              </div>
              ...
              <div className="card overflow-hidden">
                <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <h3 className="font-display font-bold text-slate-800">Rekap — {new Date(rekapMonth+'-01').toLocaleDateString('id-ID',{month:'long',year:'numeric'})}</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm min-w-[500px]">
                      ...
                      {rekapData.map(r => {
                        ...
                        return (
                          <tr key={r.student.id} className="hover:bg-slate-50/60">
                            ...
                            <td className="px-3 py-3 text-center"><span className={`px-2 py-0.5 rounded-lg font-bold ${r.alpa>0?'bg-rose-50 text-rose-700':'text-slate-300'}`}>{r.alpa}</span></td>
                            <td className="px-3 py-3 text-center font-semibold text-slate-600">{r.total||'—'}</td>
                            <td className="px-3 py-3 text-center"><span className={`font-bold ${pct===null?'text-slate-300':pct>=85?'text-emerald-600':pct>=75?'text-amber-600':'text-rose-600'}`}>{pct!==null?pct+'%':'—'}</span></td>
                          </tr>
                        )
                      })}
                    </tbody>
                    <tfoot><tr className="bg-slate-50 border-t-2 border-slate-200 text-xs font-bold text-slate-700">
                      ...
                      {['hadir','izin','sakit','alpa'].map(k=><td key={k} className="px-3 py-3 text-center">{rekapData.reduce((a,r)=>a+r[k],0)}</td>)}
                      <td className="px-3 py-3 text-center">{rekapData.reduce((a,r)=>a+r.total,0)}</td>
                      <td className="px-3 py-3 text-center">{(()=>{const h=rekapData.reduce((a,r)=>a+r.hadir,0);const t=rekapData.reduce((a,r)=>a+r.total,0);return t>0?Math.round(h/t*100)+'%':'—'})()}</td>
                    </tr></tfoot>
                  </table>
                </div>
              </div>
              ...
              {rekapDates.length > 0 && (
                <div className="card overflow-hidden">
                  <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
                    ...
                    <h3 className="font-display font-bold text-slate-800 text-sm">Detail Per Tanggal ({rekapDates.length} hari)</h3>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="text-xs min-w-max">
                      <thead><tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        {rekapDates.map(d=><th key={d} className="px-1.5 py-2.5 text-center font-semibold text-slate-500 min-w-[40px]">{fmtShort(d)}</th>)}
                      </tr></thead>
                      <tbody className="divide-y divide-slate-50">
                        {rekapData.map(r=>(
                          <tr key={r.student.id} className="hover:bg-slate-50/60">
                            <td className="px-4 py-2 font-semibold text-slate-700 sticky left-0 bg-white z-10 truncate max-w-[160px]">{r.student.name}</td>
                            {rekapDates.map(d=>{
                              const st=rekapDetail[r.student.id]?.[d]
                              return <td key={d} className="px-1.5 py-2 text-center">
                                <span className={`inline-flex items-center justify-center w-7 h-6 rounded text-xs font-bold border ${st?STATUS_CFG[st].light:'text-slate-200 border-transparent'}`}>{st?st.charAt(0):'·'}</span>
                              </td>
                            })}
                          </tr>
                        ))}
                  ...
                  <div className="px-5 py-2.5 border-t border-slate-100 flex items-center gap-3 flex-wrap">
                    {STATUSES.map(s=><span key={s} className={`text-xs font-semibold px-2 py-1 rounded-lg border ${STATUS_CFG[s].light}`}>{s.charAt(0)} = {s}</span>)}
                  </div>
                </div>
              )}
            </>
