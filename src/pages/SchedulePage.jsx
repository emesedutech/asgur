...
export default function SchedulePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data: sd, error: subjectError } = await supabase
        .from('subjects')
        .select('name')
        .eq('teacher_id', user.id)
        .order('name')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Tambahkan subject ke daftar jika belum ada
      if (!subjects.includes(form.subject.trim())) {
        ...
        // Abaikan error duplicate (UNIQUE constraint)
    ...
    <div className="space-y-5 animate-fadeUp">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Jadwal pelajaran mingguan</p>
        </div>
        <button 
                    ...
                    <div key={s.id} className="bg-white rounded-xl p-3 group shadow-sm">
                      <div className="flex items-start justify-between">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className={`text-xs font-semibold mt-1 ${DAY_TEXT[i]}`}>
                            {s.start_time?.slice(0, 5)}–{s.end_time?.slice(0, 5)}
                          </p>
                        </div>
                        <button 
                          onClick={() => handleDelete(s.id)} 
                          className="opacity-0 group-hover:opacity-100 p-1 hover:bg-rose-100 text-slate-300 hover:text-rose-600 rounded-lg transition-all shrink-0 ml-1"
                        >
                          <Trash2 size={12}/>
                        </button>
                      </div>
